...
class TestManifestUtils(unittest.TestCase):
    ...
    def test_get_host_definition_manifest_success(self):
        ...
        self.assertEqual(result[common_settings.SPEC_FIELD], expected_manifest[common_settings.SPEC_FIELD])
        self.assertEqual(result[common_settings.API_VERSION_FIELD],
                         expected_manifest[common_settings.API_VERSION_FIELD])
        self.assertEqual(result[common_settings.KIND_FIELD], expected_manifest[common_settings.KIND_FIELD])
        self.assertEqual(result[common_settings.METADATA_FIELD][common_settings.NAME_FIELD],
    ...
    def test_get_host_definition_status_manifest_success(self):
        fake_status_phase_manifest = test_manifest_utils.get_status_phase_manifest(common_settings.READY_PHASE)
    ...
    def test_get_body_manifest_for_labels_success(self):
        fake_labels_body = test_manifest_utils.get_metadata_with_manage_node_labels_manifest(
            common_settings.TRUE_STRING)
        result = manifest_utils.get_body_manifest_for_labels(common_settings.TRUE_STRING)
        self.assertEqual(result, fake_labels_body)

    def test_get_finalizer_manifest_success(self):
        fake_finalizers_manifest = test_manifest_utils.get_finalizers_manifest([common_settings.CSI_IBM_FINALIZER, ])
