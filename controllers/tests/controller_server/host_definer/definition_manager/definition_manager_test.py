...
class TestDefinitionManager(unittest.TestCase):
    def setUp(self):
        ...
        self.global_managed_nodes = test_utils.patch_nodes_global_variable(test_settings.DEFINITION_MANAGER_PATH)
    ...
    @patch('{}.manifest_utils'.format(test_settings.DEFINITION_MANAGER_PATH))
    def test_define_host_after_pending_success(self, mock_manifest_utils):
        self._test_define_host_after_pending(True, 'response', mock_manifest_utils)
        mock_manifest_utils.generate_host_definition_response_fields_manifest.assert_called_once_with(
            self.fake_host_definition_info.node_name, 'response')
        self.manager.k8s_api.patch_host_definition.assert_called_once_with(self.fake_host_definition_manifest)

    @patch('{}.manifest_utils'.format(test_settings.DEFINITION_MANAGER_PATH))
    def test_do_not_define_host_after_pending_when_node_should_not_be_managed_by_secret(self, mock_manifest_utils):
        self._test_define_host_after_pending(False, DefineHostResponse(), mock_manifest_utils)
        mock_manifest_utils.generate_host_definition_response_fields_manifest.assert_not_called()
        self.manager.k8s_api.patch_host_definition.assert_not_called()

    def _test_define_host_after_pending(self, is_node_should_be_managed, expected_response, mock_manifest_utils):
        self.manager.secret_manager.is_node_should_be_managed_on_secret.return_value = is_node_should_be_managed
        self._prepare_define_host(expected_response)
        mock_manifest_utils.generate_host_definition_response_fields_manifest.return_value = \
            self.fake_host_definition_manifest
