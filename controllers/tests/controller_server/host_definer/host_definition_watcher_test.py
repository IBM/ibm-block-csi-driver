...
@patch('controllers.servers.host_definer.watcher.host_definition_watcher.Thread')
class TestWatchHostDefinitionsResources(HostDefinitionWatcherBase):
    ...
    def test_events_on_host_definition_in_ready_state(self, mock_thread):
    ...
    def test_pending_deletion_that_managed_to_be_deleted_log_messages(self, mock_thread):
        mock_thread.side_effect = self.mock_thread_side_effect
    ...
    def test_set_error_event_on_pending_deletion(self, mock_thread):
        mock_thread.side_effect = self.mock_thread_side_effect
        self._prepare_default_mocks_for_pending_deletion()
    ...
    def test_handle_pending_host_definition_that_became_ready(self, mock_thread):
        mock_thread.side_effect = self.mock_thread_side_effect
        ...
        test_utils.patch_pending_variables()
        self.host_definition_watcher.core_api.read_node.return_value = self.k8s_node_with_fake_label
        test_utils.run_function_with_timeout(self.host_definition_watcher.watch_host_definitions_resources, 0.5)
    ...
    def test_pending_creation_that_managed_to_be_created(self, mock_thread):
        mock_thread.side_effect = self.mock_thread_side_effect
        self._prepare_default_mocks_for_pending_creation()
        self.host_definition_watcher.host_definitions_api.get.return_value = self.ready_k8s_host_definitions
        ...
        test_utils.patch_pending_variables()
        test_utils.run_function_with_timeout(self.host_definition_watcher.watch_host_definitions_resources, 0.5)
        test_utils.wait_until_called(self.host_definition_watcher.custom_object_api.patch_cluster_custom_object_status)
