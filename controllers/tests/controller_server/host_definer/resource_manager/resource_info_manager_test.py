...
class TestCSINodeManager(BaseResourceManager):
    ...
    def test_get_node_info_seccess(self):
        ...
        result = self.resource_info_manager.get_node_info(test_settings.MANAGE_NODE_LABEL)
        self.assertEqual(result.name, self.fake_node_info.name)
        self.assertEqual(result.labels, self.fake_node_info.labels)
        self.resource_info_manager.k8s_api.read_node.assert_called_once_with(test_settings.MANAGE_NODE_LABEL)
    ...
    def test_generate_host_definition_info_success(self, mock_utils):
        k8s_host_definition = test_utils.get_fake_k8s_host_definition(test_settings.READY_PHASE)
