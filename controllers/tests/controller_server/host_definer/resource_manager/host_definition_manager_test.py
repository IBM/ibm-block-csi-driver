...
class TestHostDefinitionManager(BaseResourceManager):
    def setUp(self):
        self.host_definition_manager = HostDefinitionManager()
        self.host_definition_manager.k8s_api = MagicMock()
        self.host_definition_manager.event_manager = MagicMock()
        self.host_definition_manager.resource_info_manager = MagicMock()
    ...
    def test_get_none_when_host_definition_list_empty_success(self):
        ...
        self.host_definition_manager.resource_info_manager.generate_host_definition_info.assert_not_called()
        self.host_definition_manager.k8s_api.list_host_definition.assert_called_once_with()
    ...
    @patch('{}.manifest_utils'.format(test_settings.HOST_DEFINITION_MANAGER_PATH))
    def test_create_host_definition_failure(self, mock_manifest_utils):
        ...
        mock_manifest_utils.get_finalizer_manifest.assert_not_called()
        self.host_definition_manager.resource_info_manager.generate_host_definition_info.assert_not_called()
    ...
    @patch('{}.manifest_utils'.format(test_settings.HOST_DEFINITION_MANAGER_PATH))
    def test_delete_host_definition_success(self, mock_manifest_utils):
        self._test_delete_host_definition(200, mock_manifest_utils)
        self.host_definition_manager.k8s_api.delete_host_definition.assert_called_once_with(
            test_settings.FAKE_NODE_NAME)

    @patch('{}.manifest_utils'.format(test_settings.HOST_DEFINITION_MANAGER_PATH))
    def test_fail_to_delete_host_definition_because_the_finalizers_fails_to_be_deleted(self, mock_manifest_utils):
        self._test_delete_host_definition(405, mock_manifest_utils)
        self.host_definition_manager.k8s_api.delete_host_definition.assert_not_called()

    def _test_delete_host_definition(self, finalizers_status_code, mock_manifest_utils):
        mock_manifest_utils.get_finalizer_manifest.return_value = test_manifest_utils.get_finalizers_manifest([])
    ...
    @patch('{}.utils'.format(test_settings.HOST_DEFINITION_MANAGER_PATH))
    def test_add_name_to_host_definition_info_success(self, mock_utils):
        random_string = '2049530945i3094i'
        mock_utils.get_random_string.return_value = random_string
    ...
    @patch('{}.manifest_utils'.format(test_settings.HOST_DEFINITION_MANAGER_PATH))
    def test_create_new_host_definition_success(self, mock_manifest_utils):
        host_definition_manifest = self._test_create_host_definition_if_not_exist(
            test_settings.FAKE_NODE_NAME, None, self.fake_host_definition_info, mock_manifest_utils)
        self.host_definition_manager.k8s_api.patch_host_definition.assert_not_called()
    ...
    def _test_create_host_definition_if_not_exist(self, new_host_definition_name, matching_host_definition,
        ...
        self.assertEqual(result, self.fake_host_definition_info)
        mock_manifest_utils.get_host_definition_manifest.assert_called_once_with(
