...
class TestEventManagerTest(BaseResourceManager):
    ...
    def test_generate_k8s_normal_event_success(self):
        self._test_generate_k8s_event_success(common_settings.SUCCESSFUL_MESSAGE_TYPE,
                                              common_settings.NORMAL_EVENT_TYPE)

    def test_generate_k8s_warning_event_success(self):
        self._test_generate_k8s_event_success('unsuccessful message type', common_settings.WARNING_EVENT_TYPE)
    ...
    def _test_generate_k8s_event_success(self, message_type, expected_event_type):
        result = self.event_manager.generate_k8s_event(
            ...
            common_settings.DEFINE_ACTION, message_type)
        self.assertEqual(result.metadata, test_utils.get_event_object_metadata())
        self.assertEqual(result.reporting_component, common_settings.HOST_DEFINER)
        self.assertEqual(result.reporting_instance, common_settings.HOST_DEFINER)
        self.assertEqual(result.action, common_settings.DEFINE_ACTION)
        self.assertEqual(result.type, expected_event_type)
        self.assertEqual(result.reason, message_type + common_settings.DEFINE_ACTION)
