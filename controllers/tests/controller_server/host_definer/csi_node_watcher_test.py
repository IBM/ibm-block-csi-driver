...
class CsiNodeWatcherBase(BaseSetUp):
    def setUp(self):
        ...
        self.mock_nodes_on_csi_node_watcher = test_utils.patch_nodes_global_variable(settings.CSI_NODE_WATCHER_PATH)
        self.fake_updated_daemon_set = test_utils.get_fake_k8s_daemon_set_items(1, 1)
        self.fake_not_updated_daemon_set = test_utils.get_fake_k8s_daemon_set_items(0, 1)
        self.fake_deleted_daemon_set = test_utils.get_fake_k8s_daemon_set_items(0, 0)
        self.mock_secret_ids_on_csi_node_watcher = test_utils.patch_secret_ids_global_variable(
...
class TestAddInitialCsiNodes(CsiNodeWatcherBase):
    def test_host_not_defined_for_csi_node_without_ibm_block_provider(self):
        ...
        self.assertEqual(0, len(self.mock_nodes_on_watcher_helper))
...
class TestWatchCsiNodesResources(CsiNodeWatcherBase):
    def _prepare_mocks_for_updated_csi_node(self):
        self.mock_nodes_on_watcher_helper[settings.FAKE_NODE_NAME] = settings.FAKE_NODE_ID
    ...
    def test_updated_node_id_of_csi_node(self):
        self._prepare_mocks_for_updated_csi_node()
        host_definitions = self.fake_ready_k8s_host_definitions
    ...
    def test_delete_host_definition(self):
        ...
        test_utils.run_function_with_timeout(self.csi_node_watcher.watch_csi_nodes_resources, 0.5)
        self.csi_node_watcher.host_definitions_api.delete.assert_called()
    ...
    def test_csi_node_deleted_with_modify_event(self):
        self._default_mock_csi_node_pod_deletion()
        self.csi_node_watcher.csi_nodes_api.watch.return_value = iter(
            [test_utils.get_fake_csi_node_watch_event(settings.MODIFIED_EVENT_TYPE)])
        test_utils.run_function_with_timeout(self.csi_node_watcher.watch_csi_nodes_resources, 0.5)
        self.csi_node_watcher.host_definitions_api.delete.assert_called()
    ...
    def _default_mock_csi_node_pod_deletion(self):
        ...
        self.csi_node_watcher.core_api.read_node.side_effect = [
            self.fake_k8s_node_with_manage_node_label,
            self.fake_k8s_node_with_fake_label, self.fake_k8s_node_with_fake_label]
    ...
    def test_define_host_called_on_new_csi_node(self):
        ...
        self.csi_node_watcher.host_definitions_api.get.side_effect = [
            test_utils.get_empty_k8s_host_definitions(), self.fake_ready_k8s_host_definitions]
        test_utils.run_function_with_timeout(self.csi_node_watcher.watch_csi_nodes_resources, 0.5)
        self.assertEqual(1, len(self.mock_nodes_on_watcher_helper))
        self.csi_node_watcher.storage_host_servicer.define_host.assert_called()
