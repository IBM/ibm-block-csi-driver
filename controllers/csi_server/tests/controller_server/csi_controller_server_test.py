...
import controllers.csi_server.controller_server.config as config
import controllers.csi_server.controller_server.errors as controller_errors
from controllers.csi_server.array_action.array_action_types import Host
from controllers.csi_server.array_action.array_mediator_xiv import XIVArrayMediator
from controllers.csi_server.controller_server.csi_controller_server import CSIControllerServicer
from controllers.csi_server.controller_server.sync_lock import SyncLock
from controllers.csi_server.tests.controller_server.test_settings import (CLONE_VOLUME_NAME,
                                                                          OBJECT_INTERNAL_ID,
                                                                          ...
                                                                          SNAPSHOT_VOLUME_WWN)
from controllers.csi_server.tests import utils
from controllers.csi_server.tests.utils import ProtoBufMock
...
class TestCreateVolume(BaseControllerSetUp, CommonControllerTest):
    ...
    def create_volume_returns_error(self, storage_agent, create_volume, return_code, err):
        ...
        self.assertIn(msg, self.context.details)
        self.mediator.get_volume.assert_called_once_with(VOLUME_NAME, POOL, False)
        self.mediator.create_volume.assert_called_once_with(VOLUME_NAME, self.capacity_bytes, None,
                                                            POOL, IO_GROUP, VOLUME_GROUP, False)
    ...
    def _test_create_volume_parameters(self, final_name="default_some_name", space_efficiency=None):
        ...
        self.servicer.CreateVolume(self.request, self.context)
        self.assertEqual(self.context.code, grpc.StatusCode.OK)
        self.mediator.create_volume.assert_called_once_with(final_name, 10, space_efficiency,
                                                            POOL, IO_GROUP, VOLUME_GROUP, False)
