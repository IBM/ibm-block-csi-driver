...
from controllers.servers.host_definer.hd_types import SecretInfo
...
class SecretWatcher(Watcher):
    ...
    def watch_secret_resources(self):
        while self._loop_forever():
            ...
            with watch.Watch() as w:
                stream = w.stream(self.core_api.list_secret_for_all_namespaces,
    ...
    def _generate_k8s_secret_to_secret_info(self, k8s_secret, nodes_with_system_id=None, system_ids_topologies=None):
        return SecretInfo(
            k8s_secret.metadata.name, k8s_secret.metadata.namespace,
