...
def csi_replication_method(error_response_type, lock_replication_request_attribute=''):
    @decorator
    def call_csi_method(controller_method, servicer, request, context):
        replication_id = getattr(request, lock_replication_request_attribute, None)
        if replication_id:
            if replication_id.HasField(VOLUME_GROUP_TYPE_NAME):
                ...
                lock_id = None
        return _set_sync_lock(lock_id, lock_replication_request_attribute, error_response_type,
                              controller_method, servicer, request, context)
