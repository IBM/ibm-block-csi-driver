...
func (e *Executer) CommandContext(ctx context.Context, cmdName string, args ...string) utilexec.Cmd {
	...
	// This avoids duplicating the Go runtime's heap during the fork.
	osCmd.SysProcAttr = &syscall.SysProcAttr{
		Cloneflags: syscall.CLONE_VFORK | syscall.CLONE_VM,
	}
...
// executerCmd wraps os/exec.Cmd to satisfy utilexec.Cmd interface
...
func (c *executerCmd) CombinedOutput() ([]byte, error) {
	return c.execute(false)
}

// Run executes the command but does not return output.
// mount-utils often uses this for "fire-and-forget" formatting or cleanup.
func (c *executerCmd) Run() error {
	_, err := c.execute(false)
	return err
}

// Output executes the command and returns only Stdout.
func (c *executerCmd) Output() ([]byte, error) {
	return c.execute(true)
}

// Internal helper to centralize RHEL 7 safety logic
func (c *executerCmd) execute(captureStdoutOnly bool) ([]byte, error) {
	if c.Cmd.Stdout != nil || c.Cmd.Stderr != nil {
		return nil, errors.New("exec: Stdout already set")
	}

	// 1. SET WAITDELAY (Go 1.20+)
	// This is the "Nuclear Option" that releases the Go routine even if
	// the kernel hangs the OS thread in D-state.
	c.Cmd.WaitDelay = 2 * time.Second

	var b bytes.Buffer
	// Apply your limit writer to prevent OOM from chatty mkfs output
	limitWriter := &limitWriter{Writer: &b, Limit: 1024 * 1024} // 1MB

	if captureStdoutOnly {
		c.Cmd.Stdout = limitWriter
		// Stderr is directed to the system log or ignored depending on requirements
	} else {
		// CombinedOutput style
		c.Cmd.Stdout = limitWriter
		c.Cmd.Stderr = limitWriter
	}

	// 2. START THE PROCESS (Uses VFORK from CommandContext)
	err := c.Cmd.Start()
	if err != nil {
		return nil, err
	}
	...
	// 5. SUCCESS: Clear tracking for this specific hardware
	if c.device != "" {
		//c.executer.stuckDevices.Delete(c.canonicalID)
		c.executer.clearTracking(c.device)
	}
...
// Ensure interfaces are fully implemented at compile time
var _ utilexec.Interface = &Executer{}
var _ utilexec.Cmd = &executerCmd{}
...
func (e *LightExecuter) getCanonicalID(path string) (string, error) {
	...
	if err := syscall.Stat(realPath, &stat); err != nil {
		return "", err
	}

	// Calculate Major:Minor from rdev (Standard Linux/RHEL 7 logic)
...
func (e *Executer) extractRealDevice(cmdName string, args []string) string {

// Supported commands below.
// In the current implementation, where this is used only by SafeFormatAndMount, we only
// exepct mkfs.*, blkid, fsck, e2fsck, resize2fs, xfs_repair, xfs_growfs
// mount, mkfs.ext4, mkfs.xfs, blkid
//   For these tools, the device is almost always the first non-flag argument
//   or the very first argument. Pattern: /dev/xxx
// fsck, e2fsck, resize2fs, xfs_repair
//   These tools almost always take the device path as the last argument
//   or the only non-flag argument.
// xfs_growfs"
// xfs_growfs typically takes a mount point.
//   We resolve that mount point back to its source device.
// umount, umount.xfs, umount.ext4"

	var rawPath string

	// 1. IDENTIFY THE TARGET STRING
	switch cmdName {
	case "multipathd", "multipath":
		// Pattern: multipathd show multipath <mpath_name>
		for i, arg := range args {
			if (arg == "multipath" || arg == "map" || !strings.HasPrefix(arg, "-")) && i < len(args) {
				// Handle raw alias (mpatha) vs full path (/dev/mapper/mpatha)
				if !strings.Contains(arg, "/") {
					rawPath = "/dev/mapper/" + arg
				} else {
					rawPath = arg
				}
				break
	...
	case "iscsiadm":
		// Pattern: iscsiadm -m session -T <iqn>
		// We extract the IQN and map it to a leaf device (sdX) using sysfs
		for i, arg := range args {
			if (arg == "-T" || arg == "--targetname") && i+1 < len(args) {
				iqn := args[i+1]
				// Custom helper that traverses /sys/class/iscsi_session to find the sdX
				rawPath = e.getLeafFromIqn(iqn)
				break
			}
		}
	default:
		// Generic: Find the first argument that looks like a device path or alias
		// Matches: /dev/sdX, /dev/dm-X, /dev/mapper/X, or even a raw alias if validated
		for _, arg := range args {
			if strings.HasPrefix(arg, "/dev/") {
				rawPath = arg
				break
			}
			// Prefix Inference: Handle "mpatha" passed without /dev/mapper/
			if !strings.HasPrefix(arg, "-") && !strings.Contains(arg, "/") {
				testPath := "/dev/mapper/" + arg
	...
	// 2. RESOLVE TO CANONICAL HARDWARE ID
	// Resolves symlinks (/dev/disk/by-id/...) to real device nodes (/dev/sdX)
	realPath, err := filepath.EvalSymlinks(rawPath)
	if err != nil {
		return rawPath // Return original if resolution fails
	}

	// 3. EXTRACT MAJOR:MINOR (The "Source of Truth" for D-state)
	// This is the key used for e.executer.stuckDevices.Store/Load
	var stat syscall.Stat_t
	if err := syscall.Stat(realPath, &stat); err != nil {
		return realPath
	}

	// Encode as "Major:Minor" string
	major := (stat.Rdev >> 8) & 0xfff
	minor := (stat.Rdev & 0xff) | ((stat.Rdev >> 12) & 0xfff00)
...
// IsDeviceStuck checks if any process is currently in D-state waiting on this device
func (e *Executer) IsDeviceStuck(canonicalKey string) bool {
    files, _ := os.ReadDir("/proc")
    for _, f := range files {
        if !f.IsDir() { continue }
        pid := f.Name()

        // Read /proc/[pid]/stat for state 'D'
...
func (e *Executer) isProcessWaitingOnDevice(pid string, canonicalKey string) bool {
    // 1. Scan the file descriptors of the stuck process
    fdPath := filepath.Join("/proc", pid, "fd")
    fds, err := os.ReadDir(fdPath)
    if err != nil { return false }

    for _, fd := range fds {
        // 2. Get the device ID for the file/device the process is touching
        var stat syscall.Stat_t
        if err := syscall.Stat(filepath.Join(fdPath, fd.Name()), &stat); err == nil {
            major := (stat.Rdev >> 8) & 0xfff
            minor := (stat.Rdev & 0xff) | ((stat.Rdev >> 12) & 0xfff00)
            procDev := fmt.Sprintf("%d:%d", major, minor)

            // 3. If the process has an open handle to our stuck Major:Minor,
            // it's almost certainly the cause of the D-state.
            if procDev == canonicalKey {
                return true
            }
        }
    }
...
func (e *Executer) SafeMultipathdCmd(command string) (string, error) {
	...
	// 4. Level 4: Execution
	return e.MultipathdCmd(command)
}
...
// TODO if strings.Contains(err.Error(), "unreachable") || strings.Contains(err.Error(), "refused") {

// Replace neterr:
//var netErr net.Error
// errors.As "digs" through your custom strings to find the net.Error
//if errors.As(err, &netErr) && netErr.Timeout() {
//    return false, fmt.Errorf("multipathd is unresponsive: %w", err)
//}
//if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
//    // This is where you might generate your own "deadlock suspected" error
//    return false, fmt.Errorf("multipathd deadlock detected: %w", err)
//}
