...
func NewDriver(endpoint string, configFilePath string, hostname string) (*Driver, error) {
	...
	return &Driver{
		endpoint:    endpoint,
		config:      configFile,
		NodeService: NewNodeService(configFile, hostname, *NewNodeUtils(executer, mounter, osDeviceConnectivityHelper), osDeviceConnectivityMapping, osDeviceConnectivityHelper, executer, mounter, syncLock),
	}, nil
}
