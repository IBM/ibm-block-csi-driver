...
func (d *NodeService) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	logger.Verbosef(5, ">>>> NodeStageVolume [goid=%d]: called with args %+v", util.GetGoID(), *req)
	defer logger.Verbosef(5, "<<<< NodeStageVolume [goid=%d]", util.GetGoID())
	...
	logger.Verbosef(4, "Discovered device : {%v}", device)
	if err != nil {
	...
	logger.Verbosef(4, "NodeStageVolume Finished: multipath device is ready [%s] to be mounted by NodePublishVolume API.", baseDevice)
	return &csi.NodeStageVolumeResponse{}, nil
}
...
func (d *NodeService) NodeUnstageVolume(ctx context.Context, req *csi.NodeUnstageVolumeRequest) (*csi.NodeUnstageVolumeResponse, error) {
	logger.Verbosef(5, ">>>> NodeUnstageVolume [goid=%d]: called with args %+v", util.GetGoID(), *req)
	defer logger.Verbosef(5, "<<<< NodeUnstageVolume [goid=%d]", util.GetGoID())
	...
	logger.Verbosef(4, "Reading stage info file")
	...
	logger.Verbosef(4, "Reading stage info file detail : {%v}", infoMap)

	connectivityType := infoMap["connectivity"]
	mpathDevice := infoMap["mpathDevice"]
	sysDevices := strings.Split(infoMap["sysDevices"], ",")

	logger.Verbosef(4, "Got info from stageInfo file. connectivity : {%v}. device : {%v}, sysDevices : {%v}", connectivityType, mpathDevice, sysDevices)
	...
	logger.Verbosef(4, "NodeUnStageVolume Finished: multipath device removed from host")

	return &csi.NodeUnstageVolumeResponse{}, nil
}

func (d *NodeService) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	logger.Verbosef(5, ">>>> NodePublishVolume [goid=%d]: called with args %+v", util.GetGoID(), *req)
	defer logger.Verbosef(5, "<<<< NodePublishVolume [goid=%d]", util.GetGoID())
	...
	logger.Verbosef(4, "stagingPath : {%v}, targetPath : {%v}", stagingPath, targetPath)
	...
	logger.Verbosef(4, "Got info from stageInfo file. device : {%v}", mpathDevice)

	logger.Verbosef(5, "Check if targetPath {%s} exist in mount list", targetPath)
	mountList, err := d.mounter.List()
	for _, mount := range mountList {
		logger.Verbosef(6, "Check if mount path({%v}) [with device({%v})] is equel to targetPath {%s}", mount.Path, mount.Device, targetPath)
	...
	if _, err := os.Stat(targetPathWithHostPrefix); os.IsNotExist(err) {
		logger.Verbosef(4, "Target path directory does not exist. creating : {%v}", targetPathWithHostPrefix)
		d.mounter.MakeDir(targetPathWithHostPrefix)
	}

	logger.Verbosef(4, "Mount the device with fs_type = {%v} (Create filesystem if needed)", fsType)

	logger.Verbosef(5, "FormatAndMount start [goid=%d]", util.GetGoID())
	...
	logger.Verbosef(5, "FormatAndMount end [goid=%d]", util.GetGoID())

	logger.Verbosef(4, "NodePublishVolume Finished: multipath device is now mounted to targetPath.")
...
func (d *NodeService) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	logger.Verbosef(5, ">>>> NodeUnpublishVolume [goid=%d]: called with args %+v", util.GetGoID(), *req)
	defer logger.Verbosef(5, "<<<< NodeUnpublishVolume [goid=%d]", util.GetGoID())
	...
	logger.Verbosef(5, "NodeUnpublishVolume: unmounting %s", target)
...
func (d *NodeService) NodeGetCapabilities(ctx context.Context, req *csi.NodeGetCapabilitiesRequest) (*csi.NodeGetCapabilitiesResponse, error) {
	logger.Verbosef(5, ">>>> NodeGetCapabilities [goid=%d]: called with args %+v", util.GetGoID(), *req)
	defer logger.Verbosef(5, "<<<< NodeGetCapabilities [goid=%d]", util.GetGoID())
...
func (d *NodeService) NodeGetInfo(ctx context.Context, req *csi.NodeGetInfoRequest) (*csi.NodeGetInfoResponse, error) {
	logger.Verbosef(5, ">>>> NodeGetInfo: called with args %+v", *req)
	defer logger.Verbosef(5, "<<<< NodeGetInfo")
	...
	logger.Verbosef(4, "node id is : %s", nodeId)
