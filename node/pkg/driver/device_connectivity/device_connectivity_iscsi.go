...
func (r OsDeviceConnectivityIscsi) filterLoggedIn(ctx context.Context, portalsByTarget map[string][]string) (map[string][]string, error) {
	...
	for targetName, portals := range portalsByTarget {
		...
		for _, portal := range portals {
			// CRITICAL: Normalize the portal string to "IP:Port" 
			// to match the format generated by iscsiGetRawSessions
			normalizedPortal := r.normalizePortal(portal)
			
			logger.Errorf("Normalized portal %s", normalizedPortal)

			// Check if the specific Target+Portal combination is active
			if !exists || !activePortals[normalizedPortal] {
				logger.Infof("Target %s portal %s is not logged in", targetName, portal)
				filteredPortalsByTarget[targetName] = append(filteredPortalsByTarget[targetName], portal)
			} else {
				logger.Infof("Target %s portal %s is already logged in", targetName, portal)
			}
		}
	}
...
func (r OsDeviceConnectivityIscsi) discoverAndLogin(ctx context.Context, portalsByTarget map[string][]string) {
		...
		for _, portal := range requestedPortals {
			...
			if !dbCache[targetName][normPortal] {
				if !discoveredPortals[normPortal] {
					...
					// Assuming iscsiDiscover handles its own secrets/context
					if err := r.iscsiDiscover(ctx, normPortal); err == nil {
						discoveredPortals[normPortal] = true

						// RE-SYNC: Instead of manually setting true, re-read the nodes dir
						// This confirms the discovery actually worked and populated the DB
						updatedDB := r.loadRelevantTargets(map[string][]string{targetName: {portal}})
						if updatedDB[targetName] != nil {
							if dbCache[targetName] == nil {
								dbCache[targetName] = make(map[string]bool)
							}
							for k, v := range updatedDB[targetName] {
								dbCache[targetName][k] = v
							}
						}
					}
				}
	...
	for targetName, portals := range portalsByTarget {
		for _, portal := range portals {
			// Assuming iscsiLogin handles its own logic/retries
