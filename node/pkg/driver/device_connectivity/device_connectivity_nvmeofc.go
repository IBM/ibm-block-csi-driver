...
func NewOsDeviceConnectivityNvmeOFc(executer executer.ExecuterInterface, KeyedGater *executer.KeyedGater, Mounter *mount.Mounter, clean_scsi_device bool) OsDeviceConnectivityInterface {
	// For NVMe, we create a helper but pass nil for KeyedGater and Mounter to maintain v1.13.2 behavior
	// The helper will handle nil values gracefully for NVMe operations
	helper := &OsDeviceConnectivityHelperScsiGeneric{
		Executer:        executer,
		KeyedGater:      nil, // NVMe doesn't use gating in v1.13.2
		Mounter:         nil, // NVMe doesn't use mounter in v1.13.2
		Helper:          NewOsDeviceConnectivityHelperGeneric(executer, nil, nil),
...
func (r OsDeviceConnectivityNvmeOFc) EnsureLogin(_ context.Context, ipsByArrayInitiator map[string][]string) {
...
func (r OsDeviceConnectivityNvmeOFc) ValidateLun(ctx context.Context, targetDm string, lun int, sysDevices []string, expectedSerial string) error {
