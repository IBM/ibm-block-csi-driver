...
func NewOsDeviceConnectivityFcForTest(
	executer executer.ExecuterInterface,
	helper device_connectivity.OsDeviceConnectivityHelperFcInterface,
	...
	return &device_connectivity.OsDeviceConnectivityFc{
		Executer:          executer,
		Helper:            helper,
...
func TestGetMpathDevice_Fc(t *testing.T) {
	testCases := []struct {
		name             string
...

		{
			...
			waitForPathToExistReturns: []WaitForPathToExistReturn{
				WaitForPathToExistReturn{
					devicePaths: []string{"/dev/disk/by-path/pci-fc-ID1-lun-1"},
					exists:      true,
					err:         nil,
				},
			},
			getMultipathDiskReturns: []GetMultipathDiskReturn{
				GetMultipathDiskReturn{
					pathParam: "/dev/disk/by-path/pci-fc-ID1-lun-1",
					path:      "",
					err:       fmt.Errorf("error"),
				},
			},
...

		{
			...
			waitForPathToExistReturns: []WaitForPathToExistReturn{
				WaitForPathToExistReturn{
					devicePaths: []string{"/dev/disk/by-path/pci-fc-ID1-lun1", "/dev/disk/by-path/pci-fc-ID1-lun1___2"},
					exists:      true,
					err:         nil,
				},
			},
			getMultipathDiskReturns: []GetMultipathDiskReturn{
				GetMultipathDiskReturn{
					pathParam: "/dev/disk/by-path/pci-fc-ID1-lun1",
					path:      "dm-1",
					err:       nil,
				},
				GetMultipathDiskReturn{
					pathParam: "/dev/disk/by-path/pci-fc-ID1-lun1___2",
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			fake_helper := mocks.NewMockOsDeviceConnectivityHelperFcInterface(mockCtrl)
			...
			o := NewOsDeviceConnectivityFcForTest(fake_executer, fake_helper, fake_helper_scsi_generic)
