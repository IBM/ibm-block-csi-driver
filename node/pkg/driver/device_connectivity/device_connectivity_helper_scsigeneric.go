		...
		if entries, readErr := os.ReadDir(slavesDir); readErr == nil && len(entries) > 0 {
			for _, entry := range entries {
				if strings.HasPrefix(entry.Name(), "nvme") {
					...
					hasDevice, isPending, _ := helper.EvaluateSysfsTopology(ctx, r.KeyedGater, expectedSerial, false)
...
func (r *OsDeviceConnectivityHelperScsiGeneric) ValidateLun(ctx context.Context, targetDm string, expectedLun int, sysDevices []string, expectedSerial string) error {
	...
	for _, deviceName := range sysDevices {
		...
		if isNvmePath {
			...
			state, err := secureReadSysfs(ctx, deviceName, fmt.Sprintf("/sys/block/%s/device/state", deviceName))
			if err != nil || state != "live" {
				logger.Warningf("NVMe path %s unavailable (state: %s, err: %v); skipping track", deviceName, state, err)
				continue
			}

			rawNsid, err := secureReadSysfs(ctx, deviceName, fmt.Sprintf("/sys/block/%s/device/nsid", deviceName))
			if err != nil {
				continue
			}
			...
			sysfsIdRaw, _ = secureReadSysfs(ctx, deviceName, fmt.Sprintf("/sys/block/%s/wwid", deviceName))
			if sysfsIdRaw == "" {
				sysfsIdRaw, _ = secureReadSysfs(ctx, deviceName, fmt.Sprintf("/sys/block/%s/device/wwid", deviceName))
			}

			// If the standard fabric WWID targets are missing, read the device's hardware asset serial
			var isSerialFallback bool
			if sysfsIdRaw == "" {
				sysfsIdRaw, _ = secureReadSysfs(ctx, deviceName, fmt.Sprintf("/sys/block/%s/device/serial", deviceName))
			...
			rawScsiLun, err := secureReadSysfs(ctx, deviceName, fmt.Sprintf("/sys/block/%s/device/lun", deviceName))
			if err == nil {
...
func (r *OsDeviceConnectivityHelperScsiGeneric) purgeScsiGhosts(ctx context.Context, expectedSerial string, expectedLun int, arrayIdentifiers []string) error {
	...
	for _, entry := range sgEntries {
		...
		vendorBytesRaw, err := secureReadSysfs(ctx, sgName, filepath.Join(deviceDir, "vendor"))
		if err != nil {
...
func (r *OsDeviceConnectivityHelperScsiGeneric) purgeNvmeGhosts(ctx context.Context, expectedSerial string, expectedLun int, arrayIdentifiers []string) error {
	...
	for _, entry := range blockEntries {
		...
		sysfsIdRaw, err := secureReadSysfs(ctx, name, filepath.Join("/sys/block", name, "wwid"))
		if err != nil || sysfsIdRaw == "" {
			sysfsIdRaw, _ = secureReadSysfs(ctx, name, filepath.Join(deviceDir, "wwid"))
		}

		if sysfsIdRaw == "" {
			// FIX: Shield state lookup against kernel thread deadlocks
			state, err := secureReadSysfs(ctx, name, filepath.Join(deviceDir, "state"))
		...
		func(wCtx context.Context) (struct{}, error) {
			...
			if ctrlName != "" {
				...
				if _, err := os.Stat(pciUeventPath); err == nil {
					...
					ueventStr, err := secureReadSysfs(wCtx, ctrlName, pciUeventPath)
					if err == nil {
...
func (r *OsDeviceConnectivityHelperScsiGeneric) isNvmeGhost(ctx context.Context, nvmeName string) bool {
	...
	stateStr, err := secureReadSysfs(ctx, nvmeName, path)
	if err != nil {
		if os.IsNotExist(err) {
			return true
		}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) PruneNvmeGhosts(ctx context.Context, expectedWWID string, arrayNqns []string) error {
	...
	for _, entry := range entries {
		...
		nqnData, err := secureReadSysfs(ctx, name, subsysNqnPath)
		if err != nil {
			continue 
		}
		...
		if isGhost {
			...
			state, _ = secureReadSysfs(ctx, name, filepath.Join(deviceDir, "state"))
		}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) resolveTargetIDsWithContext(ctx context.Context, baseDeviceName string) ([]string, error) {
	return executer.ExecuteUninterruptible[[]string](
		...
		func(wCtx context.Context) ([]string, error) {
			...
			return r.resolveTargetIDs(ctx, baseDeviceName)
		},
	)
}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) getNvmeSubsysNQN(ctx context.Context, deviceName string) (string, error) {
	...
	dataStr, err := secureReadSysfs(ctx, deviceNode, nqnPath)
	if err != nil {
		...
		dataStr, err = secureReadSysfs(ctx, deviceName, nqnPath)
...
func (r *OsDeviceConnectivityHelperScsiGeneric) getScsiTargetID(ctx context.Context, hctl string) string {
	...
	if _, err := os.Stat(fcPath); err == nil {
		...
		if data, err := secureReadSysfs(ctx, hctl, fcPath); err == nil && data != "" {
	...
	if _, err := os.Stat(sasPath); err == nil {
		...
		if data, err := secureReadSysfs(ctx, hctl, sasPath); err == nil && data != "" {
			return strings.TrimSpace(data)
		}
	}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) getIscsiTargetName(ctx context.Context, realDevicePath string, parentTargetBase string, hostID string) string {
	...
	for curr != "/" && curr != "." {
		...
		if strings.HasPrefix(base, "session") {
			...
			if data, err := secureReadSysfs(ctx, base, targetNamePath); err == nil && data != "" {
	...
	if strings.HasPrefix(sessionBase, "session") {
		...
		if data, err := secureReadSysfs(ctx, sessionBase, targetNamePath); err == nil && data != "" {
			sigID := strings.TrimSpace(data)
	...
	for _, s := range sessions {
		...
		data, err := secureReadSysfs(ctx, sessionName, targetNamePath)
		if err != nil || data == "" {
			continue
		}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) getHardwareSerial(ctx context.Context, deviceDir string) (string, error) {
	...
	wwidBytesStr, err := secureReadSysfs(ctx, devName, wwidPath)
...
func (r *OsDeviceConnectivityHelperScsiGeneric) FindSlavesByWWID(ctx context.Context, expectedWWID string) []string {
	...
	for _, entry := range blockEntries {
		...
		if isNVMe {
			...
			if bytesStr, err := secureReadSysfs(ctx, name, filepath.Join(targetSysDir, "nguid")); err == nil && bytesStr != "" {
				discoveredID = normalizeWWID(bytesStr)
			}
			if discoveredID == "" {
				if bytesStr, err := secureReadSysfs(ctx, name, filepath.Join(targetSysDir, "uuid")); err == nil && bytesStr != "" {
					discoveredID = normalizeWWID(bytesStr)
				}
			}
			if discoveredID == "" {
				...
				if bytesStr, err := secureReadSysfs(ctx, ctrlName, filepath.Join("/sys/class/nvme", ctrlName, "wwid")); err == nil && bytesStr != "" {
					discoveredID = normalizeWWID(bytesStr)
				}
			}
		} else {
			// SCSI unique identity properties retrieval shielded from kernel thread freezes
			if bytesStr, err := secureReadSysfs(ctx, name, filepath.Join("/sys/block", name, "device", "wwid")); err == nil && bytesStr != "" {
				discoveredID = normalizeWWID(bytesStr)
			}
		}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) GetDMNameFromMinor(ctx context.Context, minor uint32) string {
	...
	for _, dmPath := range dmMatches {
		...
		devBytesStr, err := secureReadSysfs(ctx, dmDirName, devFile)
		if err != nil {
			continue
		}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) readDMNameSafe(ctx context.Context, dmDirName string) string {
	...
	nameBytesStr, err := secureReadSysfs(ctx, dmDirName, namePath)
	...
	if err != nil || nameBytesStr == "" {
		...
		nameBytesStr, _ = secureReadSysfs(ctx, dmDirName, namePath)
	}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) IdentityAwarePreScan(ctx context.Context, targetPath string, volumeId string) (string, bool, bool, bool, error) {
	...
	if hasDevice {
		if helper.IsDeviceMapper(devName) {
			if helper.GetSlaveCount(ctx, r.KeyedGater, devName) == 0 {
...
func (r *OsDeviceConnectivityHelperScsiGeneric) executeDmsetupDeferredRemove(ctx context.Context, mpathName string) error {
	// Respect incoming orchestrator context closures immediately before launching the execution
	if err := ctx.Err(); err != nil {
		return err
	}

	command := "dmsetup"
	args := []string{"remove", "--deferred", mpathName}
	
	// Define the absolute performance safety threshold (10 seconds scaled to milliseconds)
	timeoutMSeconds := 10000 

	// Shield the user-space process invocation via your timeout tracking infrastructure
	_, err := executer.ExecuteWithTimeout(timeoutMSeconds, command, args)
	if err != nil {
		logger.Errorf("[Cleanup-Topology] User-space dmsetup deferred removal failed for map %s: %v", mpathName, err)
		return fmt.Errorf("dmsetup execution failed: %w", err)
	}

	logger.Infof("[Cleanup-Topology] User-space dmsetup deferred removal successfully dispatched for map %s", mpathName)
	return nil
}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) getWWIDBySysfs(ctx context.Context, deviceName string) (string, error) {
	...
	if nvmeNamespaceRegex.MatchString(name) || strings.HasPrefix(name, "nvme") {
		...
		if _, err := secureReadSysfs(ctx, name, wwidPath); err != nil {
	...
	dataStr, err := secureReadSysfs(ctx, name, wwidPath)
	if err != nil {
...
func secureReadSysfs(ctx context.Context, devName, sysfsPath string) (string, error) {
