	...
	if len(mounts) > 0 {
		...
		res, err := executer.ExecuteUninterruptible[IdentityResult](
			...
			func(ctx context.Context) (IdentityResult, error) {
				...
				hw, _ := r.GetDeviceWWN(devicePath)
		...
		if strings.EqualFold(currentWWID, expectedWWID) && (actualHw == "" ||  strings.EqualFold(actualHw, expectedWWID)) {
			...
			if mounted, _ := r.IsMounted(targetPath); mounted {
...
func (r *OsDeviceConnectivityHelperScsiGeneric) FinalWwidPurge(expectedWWID string) error {
	...
	for _, path := range dmUUIDs {
		data, err := os.ReadFile(path)
		if err == nil && strings.Contains(r.normalizeWWID(string(data)), targetWWID) {
			...
			if err != nil {
				...
				r.logger.Warningf("Stale device deletion for %s incomplete: %v", dmName, err)
			}
		}
	}
	...
	for _, dev := range devices {
		...
		_, err := executer.ExecuteUninterruptible[struct{}](
			...
			func(ctx context.Context) (struct{}, error) {
				...
				if !strings.EqualFold(r.normalizeWWID(currentWWID), targetWWID) {
					return struct{}{}, nil
				}
				...
				defer cancel()
				_ = r.flushDeviceBuffersWithContext(flushCtx, "/dev/"+devName)
		...
		if err != nil {
			r.logger.Warningf("Purge operation for %s reached timeout or failed: %v", name, err)
		}
	}
	return nil
}
...
func (r *OsDeviceConnectivityHelperScsiGeneric) MountVolume(source, target, wwid string, options []string) error {
	...
	_, err = executer.ExecuteUninterruptible[struct{}](
		...
		func(ctx context.Context) (struct{}, error) {
			...
			err := m.MountNative(source, target, options)
			return struct{}{}, err
		},
	)
	return err
...
func (o *OsDeviceConnectivityHelperGeneric) GetMajorMinorFromSysfs(devicePath string) (major int, minor int, error) {
    ...
    if (st.Mode & syscall.S_IFMT) == syscall.S_IFCHR && strings.HasPrefix(name, "sg") {
        ...
        if len(blockEntries) > 0 {
            ...
            if sdSt, err := os.Stat(filepath.Join("/dev", sdName)); err == nil {
                major = unix.Major(uint64(sdSt.Rdev))
                minor = unix.Minor(uint64(sdSt.Rdev))
            }
        }
    }
...
func (o *OsDeviceConnectivityHelperGeneric) GetWwnByNvmeSysfs(dev string) ([]string, error) {
	...
	if nguid, err := os.ReadFile(filepath.Join(sysPath, "nguid")); err == nil {
		return []string{o.normalize(string(nguid))}, nil
	}

	// Fallback to UUID
	if uuid, err := os.ReadFile(filepath.Join(sysPath, "uuid")); err == nil {
		return []string{o.normalize(string(uuid))}, nil
	}
	...
	if serial, err := os.ReadFile(filepath.Join(sysPath, "device/serial")); err == nil {
		return []string{o.normalize(string(serial))}, nil
	}
...
func (o GetDmsPathHelperGeneric) getSlaveCount(path string) int {
	...
	if strings.HasPrefix(name, "nvme") {
		...
		for _, e := range entries {
			if strings.HasPrefix(e.Name(), "nvme") { count++ }
		}
		return count
	}
	return 1
