...
def main():
    ...
    filtered_baseline_diff, filtered_templine_diff = _get_diffs_res(filtered_file_path, baseline_file_path,
                                                                    templine_file_path)
    ...
    _check_diffs_res(filtered_baseline_diff_file_path, filtered_templine_diff_file_path, filtered_baseline_diff,
                     filtered_templine_diff)


def _get_diffs_res(filtered_file_path, baseline_file_path, templine_file_path):
...
def _check_diffs_res(filtered_baseline_diff_file_path, filtered_templine_diff_file_path, filtered_baseline_diff,
                     filtered_templine_diff):
    if filtered_baseline_diff and filtered_templine_diff:
        _serialize_diffs_res(filtered_baseline_diff_file_path, filtered_baseline_diff, filtered_templine_diff_file_path,
                             filtered_templine_diff)
        exit(1)
    elif not filtered_baseline_diff and filtered_templine_diff:
        _serialize_diffs_res(filtered_templine_diff_file_path, filtered_templine_diff)
        exit(1)
    elif filtered_baseline_diff and not filtered_templine_diff:
        _serialize_diffs_res(filtered_baseline_diff_file_path, filtered_baseline_diff)
        exit(2)
    else:
        exit(0)


def _serialize_diffs_res(*args):
    index = 0
