...
from controller.controller_server.utils import get_array_connection_info_from_secrets, join_object_prefix_with_name
...
class HostDefinerServicer:
    ...
    def VerifyHostDefinitionNotOnStorage(self, request):  # pylint: disable=invalid-name
        node_id_info = NodeIdInfo(request.node_id)
        initiators = node_id_info.initiators
        try:
            array_connection_info = get_array_connection_info_from_secrets(request.system_info)
