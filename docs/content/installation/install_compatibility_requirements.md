...
2. Configure your storage system host attachment, per worker node.
    ...
    **Note:** IBM® block storage CSI driver 1.11.1 introduces dynamic host definition. For more information and installation instructions, see [Installing the host definer](install_hostdefiner.md). If this feature is not installed, the nodes are not dynamically defined on the storage system and they must be defined manually.
