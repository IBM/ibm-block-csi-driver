...
|**CSI-5231**|Low|In some cases, when the volume group selector information is updated, triggering an add or remove PVC operation, no PVC update events are emitted.S<br>**Workaround:** Perform one of the following:<br /> - Verify which PVCs are currently in the volume group, using the `kubectl describe volumegroup <vg_name>` command, and see the  the `status.pvcList` section.<br /> - Trigger an add or remove PVC operation by editing the PVC volume group label (by either adding or removing the label).|
