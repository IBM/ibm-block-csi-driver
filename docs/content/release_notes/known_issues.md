...
|**CSI-5722**|Service|In rare cases, when recreating pod with a previously used pvc, volume attachment gets stuck and needs to be manually released <br>**Workaround:** get list of volumeattachments and find the stuck one with (use "oc get volumeattachments.storage.k8s.io"), then use "oc edit" to release the attachments by deleting finalizers. Then re-create the pod .|
