...
class StorageClassWatcher(WatcherHelper):
    ...
    def watch_storage_class_resources(self):
        ...
        for event in watcher.stream(self.storage_api.list_storage_class):
            ...
            secrets = self._get_secrets_from_storage_class_when_it_has_csi_ibm_block_as_a_provisioner(event['object'])
            if event['type'] == settings.ADDED_EVENT:
                logger.info('New storageClass {}'.format(storage_class_name))
                self._handle_added_event_on_storage_class(storage_class_name, secrets)
            elif event['type'] == settings.DELETED_EVENT:
                self._handle_deleted_event_on_storage_class(secrets)
                
    def _get_secrets_from_storage_class_when_it_has_csi_ibm_block_as_a_provisioner(self, storage_class):
    ...
    def _is_storage_class_has_csi_ibm_block_as_a_provisioner(self, storage_class):
    ...
    def _get_secret_if_parameter_is_valid(self, storage_class, parameter):
        if parameter.endswith(settings.SECRET_NAME_SUBSTRING):
            ...
            return self._get_secret(
                storage_class, parameter, settings.DEPRECATED_SECRET_NAME_SUBSTRING)
        return ''
    ...
    def _handle_added_event_on_storage_class(self, storage_class_name, secrets):
        for secret in secrets:
            try:
                self._verify_csi_nodes_on_storage_when_new_secret(secret)
            except Exception as ex:
                logger.error('Failed to get secret data for storageclass {}, got: {}'.format(
