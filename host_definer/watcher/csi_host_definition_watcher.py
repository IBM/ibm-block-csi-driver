...
class CsiHostDefinitionWatcher(WatcherHelper):
    ...
    def _verify_host_on_storage_using_exponential_backoff(
            self, csi_host_definition_event):
        retries = 3
        ...
        while retries > 1:
            ...
            if csi_host_definition_name not in host_definition_objects_in_use:
                return
            try:
                return self._verify_host_state_on_storage(
                    host_definition_object)
            except Exception as ex:
                self._add_event_to_host_definition_object(
                    host_definition_object, str(ex))
                retries -= 1
